...
def query_checkbuttons( ):
    ...
    ##for cle, value, in dicoCheck.items():
        ##state = value.get()
        ##print("cle value=",cle,"/",value,"/", state,"")
        ##checked[cle]=state
    print("checked",CHECKED.get())
    return(CHECKED.get())
        
#-----------------------------------------------------------------------------------------------
# Create the window, a canvas and the mouse click event binding
root = Tk()
buttonFrame= Frame(root,height=6 )
buttonFrame.pack()
root.configure(background='olive')
b =Button (buttonFrame, text = " ", command = rePaint,bg="red")
b.pack(side=LEFT)
...
CANV.bind("<Button-1>", lambda x : rePaint())
#->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>-

log=[("rouge",1),("vert",2),("bleu",3)]
dicoCheck={}
checked={}
CHECKED = StringVar()
...
for cle, val, in log:
        aCheckB = Radiobutton(buttonFrame, 
            text=cle,
            variable=CHECKED,
            value=cle ,
            command=query_checkbuttons)

        aCheckB.pack(side=LEFT)
        print("cle, val  =",cle, val,"")

print ("dicoCheck",dicoCheck,log)
